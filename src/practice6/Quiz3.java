...
public class Quiz3 implements IAlgorithm
{
    ...
    public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters)
    {
        ...
        boolean isNonBifurcated = Boolean.parseBoolean(algorithmParameters.get("isNonBifurcated"));
        ...
        op.setInputParameter("h", netPlan.getVectorDemandOfferedTraffic(), "row");
        ...
        for(Node node : netPlan.getNodes()){
            ...
            for (Demand demand : netPlan.getDemands()){
                ...
                if (node == demand.getIngressNode())
                    op.addConstraint ("sum(xx_de(d,deltaPlus)) - sum(xx_de(d,deltaMinus)) == 1");
                else if (node == demand.getEgressNode())
                    op.addConstraint ("sum(xx_de(d,deltaPlus)) - sum(xx_de(d,deltaMinus)) == -1");
                else
                    op.addConstraint ("sum(xx_de(d,deltaPlus)) - sum(xx_de(d,deltaMinus)) == 0");
            }
        }
        ...
        for(Link link : netPlan.getLinks()){
            ...
            op.addConstraint("sum(h * xx_de(all,e)) <= u_e");
        }

        op.setObjectiveFunction("minimize", "sum(h * xx_de)");
        op.solve("glpk");
        ...
        netPlan.setRoutingFromDemandLinkCarriedTraffic(xx_de,true, true, setOfDemands);
    ...
    public List<Triple<String, String, String>> getParameters()
    {
        ...
        param.add(Triple.of("isNonBifurcated,", "false", " este parámetro es para permitir al usuario elegir si el encaminamiento está restringido o no a ser bifurcado."));
        return param;
    }
}
